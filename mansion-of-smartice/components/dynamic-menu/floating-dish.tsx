  ...
  return (
    ...
    >
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          {/* Dish plate/container - centered with max size constraint */}
          <div className="relative w-full h-full max-w-full max-h-full rounded-full overflow-hidden">
            {/* Check if image exists and is not placeholder */}
            {image && !image.includes('placeholder') && !imageError ? (
              <>
                {/* Show placeholder while loading */}
                {!imageLoaded && (
                  <div className={`absolute inset-0 bg-gradient-to-br ${placeholder.gradient} animate-pulse`}>
                    <div className="absolute inset-0 flex items-center justify-center">
            ...
            {/* Rim highlight - enhanced and properly centered */}
            <div className="absolute inset-0 rounded-full pointer-events-none" style={{
              background: 'radial-gradient(circle at 50% 50%, transparent 82%, rgba(255,255,255,0.15) 90%, rgba(255,255,255,0.3) 94%, rgba(255,255,255,0.1) 97%, transparent 100%)',
            }} />
          </div>
