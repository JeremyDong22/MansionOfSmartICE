...
export function CategorySwitcher({ selectedCategory, onCategoryChange }: CategorySwitcherProps) {
  ...
  const updateSelection = () => {
    ...
    // Update displayed category name when showing
    if (!showCategoryName) {
      setDisplayedCategoryName(categoryLabels[categories[closestIndex]]);
    }
  ...
  const snapToCenter = () => {
    ...
    if (items[closestIndex]) {
      ...
      // Update displayed name when snapping
      setDisplayedCategoryName(categoryLabels[categories[closestIndex]]);
    }
  };
  
  // Handle scroll with debounced snap (mobile only)
  const handleScroll = () => {
    // Only handle scroll on mobile, not PC
    if (!isMobile) return;
    ...
    if (scrollRef.current) {
      ...
      requestAnimationFrame(() => {
        ...
        if (items[currentIndex]) {
          ...
          // Initialize displayed category name
          setDisplayedCategoryName(categoryLabels[categories[currentIndex]]);
        ...
        <div className="relative flex justify-center">
          ...
          <div 
            ...
            style={{
              ...
              cursor: isMobile ? 'grab' : 'default', // PC shows default cursor
            ...
            onMouseDown={(e) => {
              // Disable dragging on PC - only allow clicks
              if (!isMobile) {
                e.preventDefault();
                return;
              }
            ...
            {categories.map((cat, i) => (
              <button
                ...
                onClick={(e) => {
                  ...
                  // Update displayed name and show category name
                  setDisplayedCategoryName(categoryLabels[categories[i]]);
                  setShowCategoryName(true);
                  ...
                  // Hide category name after delay
                  fadeTimeout.current = setTimeout(() => {
                    setShowCategoryName(false);
                  }, 1500);
                }}
                className={`flex-shrink-0 px-6 py-4 transition-transform ${!isMobile ? 'hover:scale-110' : ''}`}
                style={{
                  userSelect: 'none',
